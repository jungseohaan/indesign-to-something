...
- **진행 중 / 대기**:
  ...
  - **화학식 전수 조사 잔여 — 대부분 처리 완료(#162)**: (B 완료) 반응식 좌변/조각 재조립 + 표 셀 라벨 계수 흡수(2H₂·3H₂·2NH₃) + 평문 라벨 승격(2H2O) → `reassembleFragmentedChemicalReaction`/`absorbLeadingCoefficientIntoFormula`/`promoteWholeParagraphChemicalFormula`. **B2 `Cu+O₂→2CuO` 는 버그 아님** — 소스 Story_u322d1 이 "옳게 나타낸 것은?" 5지선다 퀴즈라 의도적 불균형 오답 보기(distractor). **C `N₂□□□NH₃` 만 미해결** — 계수비 답란 표("질소:수소:암모니아"=N₂:H₂:NH₃, Story_u1c0c7)의 formula+답란 Rectangle 이 연산자 없이 하나의 수식으로 뭉개진 것. SPEC-056 답란 변환 + 표 구조가 얽힌 영역이라 "올바른 출력"부터 모호 — 전용 조사 필요
...
- **통PNG 그룹 루트 하나만 줘도 시각 자식이 유실될 수 있음 — 그룹째 복제할 것**: 위 트랩의 이면. `_duplicateNestedCompletePngForExport` 는 복제 대상을 `_completePngDuplicateSourceIds`(=`exportSourceObjectIds` + 소유 텍스트프레임)로 정하는데, `exportSourceObjectIds`=[그룹 루트]면 루트는 복제 루프에서 skip 되고 **텍스트프레임만 복제·재그룹**된다 → 배경 rect·oval 같은 비텍스트 시각 자식이 통째로 빠진다. 배지가 숫자만 남고 흰 글자(Paper 색)가 실종되면 이걸 의심할 것 (과학 u1 p25 탐구①/② 배지, 알약 진보라+흰 "탐구"+진보라 숫자에서 알약·흰글자 소실). 해법: `exportSourceObjectIds`가 `[item.id]` 하나뿐이고 item 이 Group 이면 자식 재그룹 대신 `item.duplicate(spread)` 로 그룹째 복제 — 배치·배경 그대로 보존. 숨김 대상(`hiddenVisualSourceObjectIds`/out-of-scope)은 복제 전 원본에 선적용되므로 안전. SPEC-075 버튼 그룹은 item≠그룹루트라 미영향 ([SPEC-076](docs/specs/SPEC-076-inline-complete-png-group-visual-children-dropped.md))
...
- **한글 `treatAsChar` 인라인 객체는 세로 이동/중앙정렬이 불가 — 세로 위치 속성이 전부 무시된다**: `<hp:pic>`(그림)이든 `<hp:rect>`(박스)든 `treatAsChar="1"` 이면 한글은 `vertOffset`/`vertAlign`/`affectLSpacing` 을 **모두 무시**하고 "객체 바닥 = 텍스트 베이스라인"에 고정한다(offset +3000=30pt 를 줘도 안 움직임을 육안 확인). 그래서 원본이 본문 줄보다 큰 답란/마커 박스(예 p36 "❶ 법칙" 6mm 답란: 박스 17pt vs 콘텐츠 한 줄 11.35pt)는 세로 중앙정렬로 맞출 수 없고, 그 줄만 부풀거나(affectLSpacing 이 유효할 때조차 렌더 시 무시) 위 줄과 충돌한다. **HwpxImageBuilder 의 pic `vertOffset=-0.45H`(small marker lift)도 실은 무효였다.** 유일한 우회는 **박스 높이를 콘텐츠 한 줄 높이로 클램프 + 폭·여백을 같은 비율로 축소**(비율 유지)다. `InlineFrameBuilder.isSingleLineMarkerShell`(단일 단락·1~2자·중첩 없음) + `singleLineContentHeight`(첫 단락 FIXED 행간)로 답란/번호 마커에만 적용하고, 라벨 칩("지식·이해")·여러 줄 글상자는 제외한다(PR #166). **주의: 앵커 문단의 FIXED 행간(1750)은 박스를 수용하려 인플레이션된 값** — 클램프 타겟으로 쓰면 안 되고, 박스 **자체 콘텐츠**의 한 줄 높이를 써야 한다
