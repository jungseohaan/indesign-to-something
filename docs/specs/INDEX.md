...
| [SPEC-085](SPEC-085-grep-rule-precedence-upright-comma.md) | Active | GREP 규칙 우선순위(나중 규칙 승) 무시로 대문자·선분 수식이 이탤릭 오적용 + 수식 charStyle 입은 쉼표가 EH 장식 잔여로 오판·삭제(수학 u1 p24 "두 점 P, Q"→"P Q", 문서 전반). 규칙별 charStyle 추적·charStyle 동일성 경계 분할 + `sourceUpright`→`rm` 방출 + 구두점 보존 가드 + 단위(10g) 가드. 수학 diff 전수 의도 변화·과학 PASS (구현 완료, 육안 확인 대기) |
| [SPEC-084](SPEC-084-abolish-isolated-single-latin-demotion.md) | Active | 고립 단일 라틴 강등(SPEC-078 audit-A) 폐기 — 이니셜 오수식화 방지보다 단독 A/B/C·기하 점 라벨("점 A")의 수식화 차단 피해가 더 큼(사용자 결정). GREP 증거 단일 라틴은 문맥 무관 수식 유지. 수학 u1 +26 수식·소실 0, 과학 u1 이니셜 6곳 수식화 외 회귀 0 (구현 완료, 육안 확인 대기) |
