...
| [SPEC-076](SPEC-076-inline-complete-png-group-visual-children-dropped.md) | Done | 인라인 통PNG 그룹 복제 시 비텍스트 시각 자식 유실(과학 u1 p25 탐구①/② 배지 — 숫자만 남고 흰 "탐구"·배경 알약 소실) — 그룹 루트 단독 export 시 `item.duplicate` 로 그룹째 복제 (#155 머지, 8-49 회귀 PASS, 골든 재생성). 함께 dev 루프 기본 dpi 96→120 |
| [SPEC-077](SPEC-077-contextual-chem-formula-sibling-promotion.md) | Done | 문맥 기반 화학식 형제 승격(과학 u1 p26 2HCl 배지 — 첨자 없어 CaCO₃/CaCl₂ 형제와 달리 평문으로 남음) — 같은 부모 그룹(resolved parentId)에 확정 화학식이 있으면 첨자 없는 화학 라벨도 `rm 2HCl` 로 승격. over-trigger 3중 가드 (#157 머지) |
| [SPEC-078](SPEC-078-fragmented-chemical-formula-reassembly.md) | Done | 파편화된 화학식 재조립(과학 u1 p26 H₂O 라벨 "H₂ O" 로 벌어짐) — hair space(U+200A)·폰트크기·첨자 강등으로 `[수식 H][텍스트 2][수식 O]` 갈라진 것을 `rm H_{2}O` 로 재조립. 꼬리첨자(N₂H₄·H₂O₂) 흡수 + `stripAllFormulaSpaces`(U+200A 누수 차단) + audit-A 단일문자 오수식화 강등(GREP 단일 라틴 문자→BT수식, 과학자 이니셜 A/J/L/I) (#157 머지, 전수 조사 검증) |
| [SPEC-079](SPEC-079-math-variable-punctuation-merge-equation-loss.md) | Done | 발문 변수 x 수식화 유실(수학 u1 p12 "x의 값") — 원인은 리셋이 아니라 **audit-A 단일문자 강등이 진짜 변수 x 까지 텍스트로 되돌림**. 강등에서 **이탤릭 소문자(수학 변수)만 제외**(대문자 항목기호 A.B.C.R. 은 강등 유지). 수학 u1 +34 수식, 회귀 0, 영어 u1 무영향 |
| [SPEC-080](SPEC-080-grep-sangbuja-number-superscript-overtrigger.md) | Done | `상부자(이탤릭)` GREP(`\d+`)가 본문 숫자를 위첨자로 깨뜨리고 수식화도 안 됨(수학 u1 p14 "제곱하여 4가 되는 수"의 4·25). **Phase 1**(#175): isSuperscript()가 charStyle 이름만으로 위첨자 → 밑수 없으면 위첨자 무효화(위첨자 207→30). **Phase 2**(#178): GREP 상부자(이탤릭) 숫자를 StoryLoader 초입에서 ASTEquation 직접 방출(EH/BT 게이트 우회) + 본문 문맥 가드(한글·문장부호·원문자·탭 경계). 수학 719→835 수식(+116), 회귀 0, 화학식 무변경. 잔여: "3cm²" 같은 수치+단위+지수 혼합은 EH 변환기 근호 오인(별개) |
| [SPEC-081](SPEC-081-eh-math-glyph-census-redesign.md) | Done | EH/GREP 수식 글리프 **전수 조사(수학 1~5단원 3,577 수식) 기반 재설계**. 경로별로 흩어진 글리프 디코딩을 단일 관문(`EquationBuilder.sanitizeHwpScript`)으로 통일. A: `'`근호 미디코딩 112→1(`{'3} over {'2}`→√3/√2), Z: 라틴-1 누수 141→1(`ù`→°·`Ó`→²·`Ñ`→±), B: `^{V}`곱셈 45→0(→TIMES), E: `;4!;`GREP분수 5→0(→¼), D: `cm²`단위제곱 12→2(`rm cm^{2}` 병합). **336 수식 개선, 수식 개수 완전 동일(무회귀)**. 남은 것: `△^{9}△`(∽ 닮음, HWP 키워드 미상) |
| [SPEC-086](SPEC-086-extraction-perf-profile.md) | Active | 풀 추출 성능 프로파일(수학 u1 40p, 945초) — 계획 313초·PNG export 257초·resolved 수집 195초 + **미계측 85초 공백**(09d→10, 구 추출도 69초)·**sourceInfoCache 0히트/4,974미스** 발견. 이전 대비 +155초는 계획 패스 성장 ~50초 + 동시 부하 ~80초로 분해. 조사 단계 |
| [SPEC-085](SPEC-085-grep-rule-precedence-upright-comma.md) | Done | GREP 규칙 우선순위(나중 규칙 승) 무시로 대문자·선분 수식이 이탤릭 오적용 + 수식 charStyle 입은 쉼표가 EH 장식 잔여로 오판·삭제(수학 u1 p24 "두 점 P, Q"→"P Q", 문서 전반). 규칙별 charStyle 추적·charStyle 동일성 경계 분할 + `sourceUpright`→`rm` 방출 + 구두점 보존 가드 + 단위(10g) 가드 + p14 후속(괄호 소실·빈박스 쉼표 비대·수식 크기 축소). 수학 diff 전수 의도 변화·과학 PASS (#204 머지). 후속 2(#210): u3 p108 `rm y=ax²` 오방출 — 이름 휴리스틱("상부자"=직립) 대신 승자 charStyle 실측 FontStyle(BoldItalic) 우선 판정, u1/u5 diff 0·u3 오적용 7건만 해제 (육안 확인 완료). 후속 3(#214): 분수/근호 품은 괄호 `left( right)` 신축 확대를 EH 방출기 전용에서 sanitize 공통 관문(`EquationBuilder`)으로 이동 — u1 p14 `(½)²` 리터럴 괄호 해소, u1 35건·u3 1건 확대·u5 diff 0 |
| [SPEC-084](SPEC-084-abolish-isolated-single-latin-demotion.md) | Done | 고립 단일 라틴 강등(SPEC-078 audit-A) 폐기 — 이니셜 오수식화 방지보다 단독 A/B/C·기하 점 라벨("점 A")의 수식화 차단 피해가 더 큼(사용자 결정). GREP 증거 단일 라틴은 문맥 무관 수식 유지. 수학 u1 +26 수식·소실 0, 과학 u1 이니셜 6곳 수식화 외 회귀 0 (#204 머지) |
| [SPEC-083](SPEC-083-answer-box-paragraph-formula-gate.md) | Done | 빈 답란 박스 □ 흡수가 ±1 인접 수식 우연(쉼표 유무)으로 갈리던 것 → 문단 단위 수식 증거 게이트(`paragraphHasFormulaEvidence`)로 통일 + □ 가 원본 박스 크기·색 반영 (수학 u1 p15). pic 16개 흡수 + 유실 빈칸 7곳 □ 복구, 과학u1 PASS (#200 머지) |
| [SPEC-082](SPEC-082-floating-text-shell-structured-math.md) | Done | floating 텍스트 셸(`buildFloatingTextShellBlocks`)이 frameVisibleText 통짜 런으로 수식 파이프라인을 우회 → EH 근호 글리프 평문 누출(수학 u1 p15 `√a`→`'a`). 복수 런 스토리는 구조화 셸 변환(`buildSourceStructuredShellTextParagraphs`) 경유로 수정 (#200 머지). 잔여 별건: `0.2H8` 순환마디점 런 그룹 탈락, `48√5cm³` 본문 근호 |
