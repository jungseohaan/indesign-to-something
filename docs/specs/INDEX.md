...
| [SPEC-083](SPEC-083-answer-box-paragraph-formula-gate.md) | Active | 빈 답란 박스 □ 흡수가 ±1 인접 수식 우연(쉼표 유무)으로 갈리던 것 → 문단 단위 수식 증거 게이트(`paragraphHasFormulaEvidence`)로 통일 (수학 u1 p15 "양의 제곱근은 □, 음의 제곱근은 [pic]"). pic 16개 흡수 + 유실 빈칸 7곳 □ 복구, 과학u1 PASS (구현 완료, 육안 확인 대기) |
| [SPEC-082](SPEC-082-floating-text-shell-structured-math.md) | Active | floating 텍스트 셸(`buildFloatingTextShellBlocks`)이 frameVisibleText 통짜 런으로 수식 파이프라인을 우회 → EH 근호 글리프 평문 누출(수학 u1 p15 `√a`→`'a`). 복수 런 스토리는 구조화 셸 변환(`buildSourceStructuredShellTextParagraphs`) 경유로 수정 (구현 완료, 육안 확인 대기). 잔여 별건: `0.2H8` 순환마디점 런 그룹 탈락, `48√5cm³` 본문 근호 |
