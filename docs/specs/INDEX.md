...
| [SPEC-080](SPEC-080-grep-sangbuja-number-superscript-overtrigger.md) | Active | `상부자(이탤릭)` GREP(`\d+`)가 본문 숫자를 위첨자로 깨뜨림(수학 u1 p14 "제곱하여 4가 되는 수"의 4·25). isSuperscript()가 charStyle 이름만으로 위첨자 → **밑수 없으면 위첨자 무효화**(MathProcessor+RunPostProcessor). 위첨자 207→30, 수식 무변경, 화학식 회귀 0. **숫자 수식화는 미완성**(별도 과제) |
