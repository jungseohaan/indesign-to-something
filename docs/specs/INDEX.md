...
| [SPEC-079](SPEC-079-math-variable-punctuation-merge-equation-loss.md) | Active | 발문 변수 x 가 인접 구두점 병합("x,")으로 수식화 유실(수학 u1 p12 "x의 값") — StoryLoader GREP 수식 리셋이 trim 길이 2 라 단일 변수로 안 봐 grepMathFont 리셋. **조사 완료, 재설계 필요** — 단순 완화 시 수식 병합 교란 회귀(a,b·A=sqrt.. 8건+). 리셋↔병합↔단일변수분리 3단계 얽힘 |
