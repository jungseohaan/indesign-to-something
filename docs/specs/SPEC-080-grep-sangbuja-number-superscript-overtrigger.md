...
### Phase 2 (숫자 수식화, #178)
- [x] 수학 u1: 수식 719 → **835 (+116)**. p14 "4·2·-2·25", p15 답 항목 "⑴64 ⑵169 ⑶0.36"
      수식화. **없어진 수식 0 (회귀 없음)**
- [x] 과학 u1: 화학식 무변경 (상부자(이탤릭) 미사용, 전수 0건)
- [x] 영어 u1: 무영향
- [ ] 한글 육안 확인

### 잔여 (별개 과제)
- "3cm² 5cm²"(수치+단위+지수 혼합, 원본 `3cmÛ`)는 EH 변환기가 EH 마커 `'` 를 근호로
  오인해 `sqrt{3cm}` 으로 오변환하고 cm 을 위첨자로 조판한다. Phase 2(순수 숫자 직접
  방출) 범위 밖 — `EHFontEquationConverter` 의 글리프 디코딩·근호 판정 문제. 별도 조사 필요
