...
public class ResolvedFrameDistributor {
    ...
    public static void distribute(ASTDocument astDoc, ResolvedData resolved) {
        ...
        injectMissingInlineSemanticTextFrames(astDoc, resolved);
    ...
    private static void injectMissingInlineSemanticTextFrames(ASTDocument astDoc, ResolvedData resolved) {
        if (astDoc == null || resolved == null || resolved.textFrames() == null) return;

        Set<String> existingSources = new HashSet<>();
        Set<String> existingStories = new HashSet<>();
        for (ASTSection section : astDoc.sections()) {
            for (ASTBlock block : section.blocks()) {
                ...
                if (tfb.sourceId() != null) existingSources.add(tfb.sourceId());
                if (tfb.storyId() != null) existingStories.add(tfb.storyId());
            }
        }

        int injected = 0;
        for (ASTSection section : astDoc.sections()) {
            ResolvedPage page = resolved.getPageByName(String.valueOf(section.pageNumber()));
            if (page == null) continue;
            for (ResolvedTextFrame tf : resolved.textFrames()) {
                if (tf == null || tf.pageIndex() != page.index()) continue;
                if (!shouldInjectMissingInlineSemanticTextFrame(tf)) continue;

                String sourceId = decimalToIdmlHex(tf.id());
                String storyId = decimalToIdmlHex(tf.storyId());
                if (sourceId == null || storyId == null) continue;
                if (existingSources.contains(sourceId) || existingStories.contains(storyId)) continue;

                ASTTextFrameBlock block = createResolvedInlineTextFrameBlock(tf, storyId, page, resolved);
                if (block == null || block.paragraphs().isEmpty()) continue;
                section.addBlock(block);
                existingSources.add(sourceId);
                existingStories.add(storyId);
                injected++;
            }
        }
        if (injected > 0) {
            System.out.println("[FrameDistributor] Injected " + injected
                    + " missing inline semantic TextFrame(s) from resolved ownership");
        }
    }

    private static boolean shouldInjectMissingInlineSemanticTextFrame(ResolvedTextFrame tf) {
        if (tf == null || !tf.isInline() || tf.onHiddenLayer() || tf.nonprinting()) return false;
        if (tf.previousFrameId() != null || tf.nextFrameId() != null) return false;
        String text = cleanVisibleText(tf.frameVisibleText());
        if (text.isEmpty()) return false;
        return !isSimplePlacementMarker(text);
    }

    private static ASTTextFrameBlock createResolvedInlineTextFrameBlock(
            ResolvedTextFrame tf,
            String storyId,
            ResolvedPage page,
            ResolvedData resolved) {
        double[] bounds = tf.pageRelativeBounds();
        if (!validBounds(bounds) && validBounds(tf.geometricBounds())) {
            double[] rel = page != null ? page.toPageRelative(tf.geometricBounds()) : null;
            if (rel != null) {
                bounds = new double[] {
                        rel[1],
                        rel[0],
                        rel[1] + (tf.geometricBounds()[2] - tf.geometricBounds()[0]),
                        rel[0] + (tf.geometricBounds()[3] - tf.geometricBounds()[1])
                };
            }
        }
        ...
        block.sourceId(decimalToIdmlHex(tf.id()));
        block.storyId(storyId);
        block.x(CoordinateConverter.pointsToHwpunits(bounds[1]));
        block.y(CoordinateConverter.pointsToHwpunits(bounds[0]));
        block.width(CoordinateConverter.pointsToHwpunits(bounds[3] - bounds[1]));
        block.height(CoordinateConverter.pointsToHwpunits(bounds[2] - bounds[0]));
        block.zOrder(tf.zOrder());
        block.columnCount(Math.max(1, tf.columnCount()));
        ...
        block.insetTop(inset(tf, 0));
        block.insetLeft(inset(tf, 1));
        block.insetBottom(inset(tf, 2));
        block.insetRight(inset(tf, 3));
        block.fillColor(resolveFrameColor(tf.fillColor(), resolved));
        block.fillTint(tf.fillTint());
        block.strokeColor(resolveFrameColor(tf.strokeColor(), resolved));
        ...
        block.frameVisibleText(tf.frameVisibleText());
        block.frameVisibleTextLength(cleanVisibleText(tf.frameVisibleText()).length());

        ResolvedStory story = resolved.getStory(tf.storyId());
        if (story != null && story.paragraphs() != null && !story.paragraphs().isEmpty()) {
            for (ResolvedParagraph resolvedPara : story.paragraphs()) {
                ASTParagraph para = createParagraphFromResolved(resolvedPara, resolved);
                if (para != null && !para.items().isEmpty()) block.addParagraph(para);
            }
        }
        if (block.paragraphs().isEmpty()) {
            ASTParagraph para = new ASTParagraph();
            ASTTextRun run = new ASTTextRun();
            run.text(cleanVisibleText(tf.frameVisibleText()));
            para.addItem(run);
    ...
    private static ASTParagraph createParagraphFromResolved(ResolvedParagraph resolvedPara, ResolvedData resolved) {
        if (resolvedPara == null) return null;
        ASTParagraph para = new ASTParagraph();
        para.paragraphStyleRef(resolvedPara.styleName());
        para.alignment(resolvedPara.justification());
        Double leading = resolvedPara.fixedLeading();
        if (leading != null && leading > 0) {
            para.lineSpacingType("fixed");
            para.lineSpacing((int) CoordinateConverter.pointsToHwpunits(leading));
        }
        ...
        for (ResolvedRun run : resolvedPara.runs()) {
            for (ASTTextRun astRun : TextRunSegmenter.fromResolvedText(
                    run != null ? run.text() : null,
                    run,
                    resolved != null ? resolved::resolveColorHex : null,
                    para.hasTabStops(),
                    true,
                    null)) {
                para.addItem(astRun);
    ...
    private static long inset(ResolvedTextFrame tf, int idx) {
        double[] insets = tf.insetSpacing();
        if (insets == null || idx < 0 || idx >= insets.length) return 0;
        return CoordinateConverter.pointsToHwpunits(insets[idx]);
    }

    private static String resolveFrameColor(String color, ResolvedData resolved) {
        if (color == null || color.isEmpty() || color.contains("None") || color.contains("[None]")) {
            return null;
        }
        String hex = resolved != null ? resolved.resolveColorHex(color) : null;
        return hex != null ? hex : color;
    }

    private static boolean isSimplePlacementMarker(String text) {
        String t = cleanVisibleText(text);
        if (t.isEmpty()) return true;
        if (t.matches("[0-9]{1,2}")) return true;
        if (t.matches("[가-힣]") || t.matches("[ㄱ-ㅎ]")) return true;
    ...
    private static String decimalToIdmlHex(String decimalId) {
        if (decimalId == null || decimalId.isEmpty()) return null;
        try {
