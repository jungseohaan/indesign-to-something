...
public class Stage4_BuildAST {
        ...
        for (IDMLSpread spread : idmlDoc.spreads()) {
            for (IDMLPage page : spread.pages()) {
                ...
                for (FlatObject fo : textFrames) {
                    ...
                    // 이미 처리된 스토리 — 스프레드 양쪽 페이지에 걸친 배경 블록이면 geometry만 재생성
                ...
                if (imageLoader != null) {
                    ...
                    ResolvedData finalResolvedData = resolvedData;
                    ResolvedPage finalResolvedPage = resolvedPage;

                    // 개별 도형 래스터화 (resolved 좌표 활용)
                    List<ASTFigure> vectorFigures = ungrouped.parallelStream()
                            .map(shape -> ASTInlineObjectBuilder.createFigureFromVectorShape(
                                    shape, finalPage2, finalImageLoader2, colorResolver,
                                    finalResolvedData, finalResolvedPage))
                            .filter(Objects::nonNull)
                            .collect(Collectors.toList());
                ...
                // === 페이지 여백 가이드라인 (맨 밑 레이어) ===
                if (imageLoader != null) {
                    ASTFigure marginGuide = createMarginGuideFigure(page);
                    if (marginGuide != null) {
     ...
     * z-order -20000으로 모든 콘텐츠 뒤에 렌더링된다.
     */
    private static ASTFigure createMarginGuideFigure(IDMLPage page) {
     ...
     */
    private static void simulateTextWrap(ASTSection section) {
